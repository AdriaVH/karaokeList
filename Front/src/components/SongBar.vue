<template>
    ...
    <div
        class=" valencia hide-scrollbar ring-gray-600 ring-opacity-30 ring-8  flex flex-col items-center overflow-scroll bg-opacity-40 bg-slate-900 h-full">
        <div class=" w-[180px]  madrid absolute -mt-9 flex flex-col transition-all duration-[2900ms] ease-in-out">
            <div
        ...
        <img @click="showingOptions = !showingOptions" src='../assets/images/keys2.png'
        ...
        <h1 v-if="karaoke.expanded" v-for="(song, index) in songBarSongs" :key="index"
            ...
            <div  v-if="index > 0" class="flex flex-row w-full justify-center mt-1">
                ...
                <div @click="selectSong(song)" class=" gap-3 items-center flex flex-row mt-1 w-[340px] rounded-lg -gap-1" :class="{
                ...
                <img @click="songDown(song, index, index + 1)"
                    v-if="showingOptions && index !== songBarSongs.length - 1" src="../assets/images/arrowRedDown.png"
                    class=" mt-1 opacity-70 h-[55px] w-[60px]" alt="">
...
import songAdded from './songAdded.vue';
export default {
    name: "SongBar",
    data() {
        return {
